...
builtins.search = function(sourceString, searchString) {
  ...
  for (var i=0; i < searchString.length; i++) {
    if (sourceString.indexOf(searchString[i]) == -1) {
      return false
    }
    if (i != 0&& (sourceString.indexOf(searchString[i]) != c -1)) {
      return false
    }
    else {
      c =sourceString.indexOf(searchString[i]) ;
    }
  }
  return true
};
...
builtins.isPalindrome = function(arr) {
  var new_Array = builtins.reverse(arr);
  return builtins.isEqual(new_Array, arr)
};
