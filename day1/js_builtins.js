...
builtins.search = function(sourceString, searchString) {
  ...
  var spot = 0;
  spot = sourceString.indexOf(searchString[0]);

  if(spot === -1){
    return false;
  }

  for(var i = spot; i < searchString.length; i++){
    if(sourceString[i] !== searchString[i]){
      return false;
    }
  }
...
builtins.isPalindrome = function(arr) {
  var flipped = builtins.reverse(arr);
  return builtins.isEqual(arr, flipped);
};
