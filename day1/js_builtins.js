...
builtins.trim = function(str) {
  // YOUR CODE HERE
  //you want the index of the first letter and the index of the last letter
  var firstindex = 0;
  var lastindex = 0;
...
builtins.isPalindrome = function(arr) {
  ...
  for (var j = arr.length - 1; j > middleindex - 2; j--) {
    newarr2.push(arr[j]);
  }
  //console.log("newarr is: " + newarr);
  //console.log("newarr2 is: " + newarr2);
  if (builtins.isEqual(newarr, newarr2)) {
    return true;
  }
  else {
    return false;
...
builtins.flatten = function(arr) {
  ...
  for (var i = 0; i < arr.length; i++) {
    for (var inner = 0; inner < arr[i].length; inner++) {
      ans.push(arr[i][inner]);
    }
  }
  return ans;
};
