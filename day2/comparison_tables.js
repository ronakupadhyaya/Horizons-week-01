...
for( var i = 0; i < valuesToCheck().length; i++){
  console.log(String(valuesToCheck()[i]));
}
  ...
  for(var i = 0; i < valuesToCheck().length; i++){
    for(var j = 0; j < valuesToCheck().length; j++){
      resultArray.push[valuesToCheck()[i],valuesToCheck()[j]];
    }
  }
...
comp.testStrictEquality = function() {
  ...
  for(var i = 0; i < valuesToCheck().length; i++){
    for(var j = 0; j < valuesToCheck().length; j++){
      ...
      resultTable[String(valuesToCheck()[i]) + '_' + String(valuesToCheck()[j])] = (valuesToCheck()[i] === valuesToCheck()[j]);
    }
  }
  return resultTable;
};
