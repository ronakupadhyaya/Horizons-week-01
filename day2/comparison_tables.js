...
comp.testLooseEquality = function() {
  ...
  for (var i = 0; i < values.length; i++) {
    for (var j = 0; j < values2.length; j++) {
      arrayCompare.push([values[i], values2[j]])
    }
  }
  for (var i = 0; i < arrayCompare.length; i ++) {
    var a = arrayCompare[i][0]
    var b = arrayCompare[i][1]
    var newString = ArrayToString(a) + '_' + ArrayToString(b)
    if (a == b) {
      objectTest[newString] = true;
    } else {
      objectTest[newString] = false;
    }
  }
  return objectTest;
};
