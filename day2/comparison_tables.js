...
comp.testLooseEquality = function() {
  ...
  for(var i = 0; i < valuesToCheck().length; i++){
    for( var j = 0; j < valuesToCheck().length; j++){
      var a;
      var b;
      if(valuesToCheck()[i] ===Infinity || valuesToCheck()[i] === -Infinity || (typeof valuesToCheck()[i] === 'number' && isNaN(valuesToCheck()[i])))
        a = valuesToCheck()[i];
      else
        a = JSON.stringify(valuesToCheck()[i]);

      if(valuesToCheck()[j] ===Infinity || valuesToCheck()[j] === -Infinity || (typeof valuesToCheck()[j] === 'number' && isNaN(valuesToCheck()[j])))
        b = valuesToCheck()[j];
      else
        b = JSON.stringify(valuesToCheck()[j]);


      var key = a+"_"+b;
      var val = (valuesToCheck()[i] == valuesToCheck()[j]);
      obj[key] = val;
    }
  }
  console.log(obj);
