...
dates.createDate = function(dateStr) {
  if (typeof parseInt(dateStr.slice(0,1)) === 'number') {
    return new Date(dateStr)
  } else {
    var arr = dateStr.split(" ")
    var hoursMinSec = arr[3].split(":")

    var hours = parseInt(hoursMinSec[0])
    var min = parseInt(hoursMinSec[1])
    var sec = parseInt(hoursMinSec[2])

    var month = "JanFebMarAprMayJunJulAugSepOctNovDec".indexOf(arr[0].slice(0,3)) / 3 + 1
    var year = parseInt(arr[2])
    var day = parseInt(arr[1].substring(0,arr[1].length-1))
