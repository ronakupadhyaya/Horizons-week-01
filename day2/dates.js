...
dates.createDate = function(dateStr) {
  ...
  dateStr[0][1] = dateStr[0][1].slice(0, dateStr[0][1].length-1)
  var dateArr = [];
  for (var i = 0; i < dateStr.length; i++) {
    for (var j = 0; j < dateStr[i].length; j++) {
      dateArr.push(dateStr[i][j])
    }
  }
...
dates.isSameTimeOfDay = function(dateObj, otherDateObj) {
  // YOUR CODE HERE
  if (dateObj.getUTCHours() === otherDateObj.getUTCHours() && dateObj.getUTCMinutes() === otherDateObj.getUTCMinutes()) {
    return true
  }
  return false
};
