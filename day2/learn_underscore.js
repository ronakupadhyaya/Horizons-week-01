...
learn_underscore.any = function(array, fun) {
  var updated = _.map(array, fun);
  var ans = _.reduce(updated, function(a,b){
    return a || b;
  });
  return ans;
}
...
learn_underscore.values = function(object) {
  var keys = [];
  _.forEach(object, function(value, key, object) {
    keys.push(value);
  }) ;
  return keys;
}
...
learn_underscore.countBy = function(array, fun) {
  var arr = _.groupBy(array, fun);
  console.log(arr);
  var other = _.mapObject(arr, function (value, key) {
    return value.length;
  });
  console.log(other);
  return other;
