...
learn_underscore.any = function(array, fun) {
	var temp = _.map(array, fun);
	return _.reduce(temp, function(a,b){return a || b;}, false);
}
...
learn_underscore.keys = function(object) {
	var ans = [];
	_.forEach(object, function(value, key){ ans.push(key);});
	return ans;
}
...
learn_underscore.values = function(object) {
	var ans = [];
	_.forEach(object, function(value, key){ ans.push(value);});
	return ans;
}
...
learn_underscore.pairs = function(object) {
	var ans = [];
	_.forEach(object, function(value, key){ ans.push([key,value]);});
	return ans;
}
...
learn_underscore.countBy = function(array, fun) {
	var obj = _.groupBy(array, function(word){ return fun(word); });
	return _.mapObject(obj, function(val, key) { return val.length; });
}
