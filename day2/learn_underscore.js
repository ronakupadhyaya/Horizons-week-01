...
learn_underscore.contains = function(array, item) {
    // value is the value that you assign each item in the array
    // item is what you use to compare
...
learn_underscore.any = function(array, fun) {
    // var maps = array.map(fun);
    var maps = _.map(array, fun);
    return _.reduce(maps, function(a, b) {
        return a || b;
    });
}
...
learn_underscore.countBy = function(array, fun) {
    var counts = _.groupBy(array, fun);
    return _.mapObject(counts, function(value, key) {
