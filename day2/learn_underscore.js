...
learn_underscore.any = function(array, fun) {
  // YOUR CODE HERE
  // use .map to identify truthyness
  //.reduce to check for true
//debugger;
  function truthy(n) {
    return fun(n);
  }
  var newArr = _.map(array, truthy);

  function compareTrue(a, b) {
    return a || b;
  }

  return _.reduce(newArr, compareTrue);
}
...
learn_underscore.keys = function(object) {
  ...
  _.forEach(object, function (value, key) { arr.push(key)} )
...
learn_underscore.values = function(object) {
  ...
  _.forEach(object, function (value, key) { arr.push(value)} )
...
learn_underscore.countBy = function(array, fun) {
  // YOUR CODE HERE
  // map --> word to word.length
  return _.mapObject(_.groupBy(array, fun), function(value, key){
    return value.length;
  }
)
  };

  //var newArr = _.map(array, fun);
