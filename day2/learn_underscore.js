...
learn_underscore.any = function(array, fun) {
  // YOUR CODE HERE
  var testArr = _.map(array, function(item){
    return fun(item) ? true : false;
  });

  return _.reduce(testArr, function(a,b){
    return a || b;
  });
}
...
learn_underscore.countBy = function(array, fun) {
  // YOUR CODE HERE
  var newArr = _.groupBy(array, fun);
  return _.mapObject(newArr, function(val, key){
    return val.length;
  });
}
