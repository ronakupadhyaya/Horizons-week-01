...
learn_underscore.any = function(array, fun) {
  // YOUR CODE HERE
  var modifiedArray = _.map(array, function(item){
    return fun(item);
  });
  return _.reduce(modifiedArray, trueOrFalse);
  function trueOrFalse(a, b){
    return a || b;
  }
  //_.reduce([false, false, true], trueOrFalse)
...
learn_underscore.keys = function(object) {
  // YOUR CODE HERE
  var objectArray = [];
  _.forEach(object, function(item, item2){
    //console.log(item2)
    objectArray.push(item2);
  });
  return objectArray;
}
...
learn_underscore.pairs = function(object) {
  // YOUR CODE HERE
  var objectArray = [];
  _.forEach(object, function(item1, item2){
    objectArray.push([item2, item1]);
  });
  return objectArray
}
...
learn_underscore.countBy = function(array, fun) {
  ...
  //var test = [];
  //console.log(fun);
  var object = _.groupBy(array, fun);
    //test.push(fun(item));
    //return fun(item);
  var result = _.mapObject(object, function(value, key){
    return value.length;
  });
  console.log(object);
  return result
