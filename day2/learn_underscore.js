...
learn_underscore.contains = function(array, item) {
   return _.any(array, function(item2) {
...
learn_underscore.any = function(array, fun) {
  var x = _.map(array, fun)
  var y = _.reduce(x, function (a,b) {
    return (!!a || !!b) === true
   })
  return y;
  }
  ...
  // /  var x = !!array_.map();
  //  x.reduce
   //combine the items in this array using reduce and then
//    console.log(x)
// }
...
learn_underscore.keys = function(object) {
  ...
  return x //but this only works if the key has a length of 1
}
...
learn_underscore.countBy = function(array, fun) {
 var x = _.mapObject(_.groupBy(array, fun), function(num) {
    return num.length
   })
   return x;
 }
