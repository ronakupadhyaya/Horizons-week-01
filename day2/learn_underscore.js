...
learn_underscore.contains = function(array, item) {
  return  _.any(array, function(arrElement) {
    return item === arrElement;
  });
};
...
learn_underscore.any = function(array, fun) {
  ...
  var functArr = _.map(array, fun);

  // Function: returns if true if any are truthy, false otherwise
  var compareTruthiness = function(a, b){
    return a || b;
  }

  // Finding a value of true or false in our array
  var isTruthy = _.reduce(functArr, compareTruthiness)

  // Returns if its truthy or not
  return isTruthy;
}
...
learn_underscore.countBy = function(array, fun) {
  ...
  return _.mapObject(groupedObj, function(value, key){
    return value.length;
  });
