...
learn_underscore.any = function(array, fun) {
  //console.log(array);
  var testItems = _.map(array, fun);
  return _.reduce(testItems, function(item1, item2) {
    return item1 || item2
  });
...
learn_underscore.countBy = function(array, fun) {
  ...
  // console.log("groupedBy", _.groupBy(array, fun));
  var returnObject = _.groupBy(array, fun);
  return _.mapObject(returnObject, function(val, key) {
    return val.length;
  });
