...
learn_underscore.any = function(array, fun) {
  var trueFalse = _.map(array,fun);
  return _.reduce(trueFalse,function(a,b){return a||b;});
};
...
learn_underscore.values = function(object) {
  var values = [];
  _.forEach(object,function(value, key){values.push(value);});
  return values;
}
...
learn_underscore.pairs = function(object) {
  var pairs = [];
  _.forEach(object,function(value, key){pairs.push([key,value]);});
  return pairs;
}
...
learn_underscore.countBy = function(array, fun) {
  ...
  var counts = _.mapObject(groups,function(value,key){return value.length;});
  return counts;
}
