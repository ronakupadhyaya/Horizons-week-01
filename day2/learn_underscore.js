...
learn_underscore.contains = function(array, item) {
  return _.any(array, function(comp) {
    return comp === item;
  })
};
...
learn_underscore.any = function(array, fun) {
  var boolTable = _.map(array, fun);
  return _.reduce(boolTable, function(a, b) {
    return a || b;
  });
}
...
learn_underscore.reduce = function(array, fun) {
  ...
  var accumulator = fun(array[0], array[1]);
  var inProgress = array.slice(2);
  inProgress.unshift(accumulator);
  return learn_underscore.reduce(inProgress, fun);
}
*/
...
learn_underscore.reduce = function(array, fun) {
  ...
  for (var i = 0; i < array.length; i++) {
    ...
    accumulator = fun(array[i], array[i+1]);
    array[i+1] = accumulator;
  }
}
...
learn_underscore.countBy = function(array, fun) {
  ...
  var ret = _.mapObject(grouped, function(value, key) {
    return value.length;
  });
  return ret;
}
