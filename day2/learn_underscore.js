...
learn_underscore.contains = function(array, item) {
  return _.any(array, function(check) {
    return item === check;
  });
};
...
learn_underscore.any = function(array, fun) {
  var retArray = _.map(array, fun);
  retArray = _.reduce(retArray, function(a,b){
    return a || b;
  })
  return retArray;
}
...
learn_underscore.countBy = function(array, fun) {
  var retArray = _.groupBy(array, fun);
  return _.forEach(retArray, function(value, key) {
    retArray[key] = value.length;
  })
  return retArray;
}
