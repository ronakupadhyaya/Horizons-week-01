...
learn_underscore.any = function(array, fun) {
  // YOUR CODE HERE
  // given initial array and function fun, map returns
  // a new array with the values where it's like the values
  // put into the function, and then reduce will combine
  // all elements in the returned array using the function fun.
  // var acc = false; 
  // var array = _.map(array, fun); 
  // THIS WORKS TOO, BUT THE OTHER IS MORE EFFICIENT
  // return _.reduce(array, function(acc, index) {
  //   if (index === true) {
  //     acc = true; 
  //   } 
  //   return acc;
  ...
  return _.reduce(_.map(array, fun), function(accumulator, item) {
...
learn_underscore.countBy = function(array, fun) {
  // YOUR CODE HERE
  var groupedArray = _.groupBy(array, fun); 
  return _.mapObject(groupedArray, function(val, key) {
    return val.length; 
  });
