...
learn_underscore.any = function(array, fun) {
  var tempArray = _.map(array, function(n){
    if (fun(n)) { return 1 }
    else { return 0 }
  })
  // KEEP IN MIND
  return !! _.reduce(tempArray, function(m,k){
    return m+k
  });
}
...
learn_underscore.countBy = function(array, fun) {
  var tempObj = _.groupBy(array, fun)
  return _.mapObject(tempObj, function(value,key){
    return value.length
  })
}
