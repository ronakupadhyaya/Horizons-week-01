...
learn_underscore.contains = function(array, item) {
  return _.any(array, function(arg) {
    return arg === item; 
  }) 
...
learn_underscore.any = function(array, fun) {
  // use .map to change elements to true/false values
  var newArray = _.map(array, function(x) {
    return fun(x) 
  })
  // use .reduce to look for true values and use || operator to reduce to true statement
  return _.reduce(newArray, function(a, b) {
    return a || b
  })
}
...
learn_underscore.countBy = function(array, fun) {
  var newGroup = _.groupBy(array, fun);
  return _.mapObject(newGroup, function(value) {
    return value.length;
  })
}
