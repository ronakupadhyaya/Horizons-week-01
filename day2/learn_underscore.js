...
learn_underscore.contains = function(array, item) {
  return _.any(array, function(letter) {
    return item === letter;
  });
};
...
learn_underscore.any = function(array, fun) {
  var mapOutput = _.map(array,fun);
  return _.reduce(mapOutput,function(a,b){
    if(a || b){
      return true;
    }else {
      return false;
...
learn_underscore.countBy = function(array, fun) {
  var newObject = _.groupBy(array, fun);

  var keyArray = learn_underscore.keys(newObject);

  for (var i =0;i<keyArray.length;i++){
    newObject[keyArray[i]] = newObject[keyArray[i]].length;
  }
