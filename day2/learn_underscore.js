...
learn_underscore.any = function(array, fun) {
  return _.reduce(_.map(array,fun), function (a,b){return(a||b)});
}
...
learn_underscore.reduce = function(array, fun) {
  while (array.length > 1) {
    var replace = fun(array[0],array[1]);
    array.splice(0, 2, replace)
  }
  return array[0];
}
...
learn_underscore.keys = function(object) {
  var arrayOfKeys = [];
  _.forEach(object, function(value, key, list) {arrayOfKeys.push(key)});
  return arrayOfKeys;
}
...
learn_underscore.values = function(object) {
  var arr = [];
  _.forEach(object, function(value, key, list) {arr.push(value)});
  return arr;
}
...
learn_underscore.pairs = function(object) {
  var arr= [];
  _.forEach(object, function(value, key, list) {arr.push([key,value])});
  return arr;
}
...
learn_underscore.countBy = function(array, fun) {
  return _.mapObject(_.groupBy(array, fun), function(value) {return value.length});
}
