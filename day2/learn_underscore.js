...
learn_underscore.contains = function(array, item) {
  return _.any(array, function(object) {
    return object === item
  });
};
...
learn_underscore.any = function(array, fun) {
  var new_array = _.map(array, function (item) {
    if (fun(item)) {
      return true;
    } else {
      return false;
  ...
  return _.reduce(new_array, function(a, b) {
    return a || b;
  });
}
...
learn_underscore.countBy = function(array, fun) {
  var myObj = _.groupBy(array, fun);
  return _.mapObject(myObj, function (val, key) {
    return val.length;
  })
}
