...
var once = function(f) {
  ...
  return function() {
    if (!called) { // if f hasn't been called yet
      returnValue = f.apply(null, arguments) // call f
...
var functionFactory = function(num1, num2) {
  var functionArray = [];
  var ind = 0;
  for (var i = num1; i <= num2; i++) { //-2, 1 // --> -2, -1, 0, 1
    functionArray[ind] = (function(i) {
      return function inner(){ //anything before this line will be captured;
