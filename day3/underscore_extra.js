...
function memoize(func) {
  var calls = []; // Let's create a local variable to track if f has been called
  return function () {
    var called = false;
    for (var i = 0; i < calls.length; i++) {
      if(calls[i][0] === arguments[0]) {
        called = true
        return calls[i][1];
      }
    }
    if (!called){
      var answer = func(arguments[0])
      calls.push([arguments[0], answer]);
    }
    return answer;
  }
}
