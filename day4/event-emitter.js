...
EventEmitter.prototype.emit = function(eventName, arg) {
	var lists = this.listeners[eventName];
  	if(lists && lists.length){
  		lists.forEach(function(fn){fn(arg);
  	});
  }
}
...
EventEmitter.prototype.removeListener = function(eventName, fn) {
 	if(eventName in this.listeners && this.listeners[eventName].indexOf(fn) > -1){
 		var i = this.listeners[eventName].indexOf(fn);
 		this.listeners[eventName].splice(i);
 	}
}
