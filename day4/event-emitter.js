...
// Takes in a string "eventName" and a single argument arg
...
EventEmitter.prototype.emit = function(eventName, arg) {
  ...
  var functionsToCall = this.listeners[eventName];
  functionsToCall.forEach(function(fun) {
    fun(arg);
  });
}
...
EventEmitter.prototype.removeListener = function(eventName, fn) {
  //assuming fn is the same referenced function
  if(!this.listeners.hasOwnProperty(eventName)) {
    return;
  }
  debugger;
  ...
  //var foundFn = false;
  var fnIndex = listeners.indexOf(fn);
  if(fnIndex === -1) {
    throw "Given fn was not found in listeners for " + eventName + ".";
  }
...
EventEmitter.prototype.once = function(eventName, fn) {
  this.on(eventName, newFunction);
  var self = this;
  function newFunction(arg) {
    self.removeListener(eventName, newFunction);
    fn(arg);
  }
}
