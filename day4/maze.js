...
Maze.prototype.tryMove = function(row, column, direction) {
  ...
  if (move[0] < 0 ||
    move[0] >= this.maze.length ||
    move[1] < 0 ||
    move[1] >= this.maze[0].length ||
    this.maze[move[0]][move[1]]==="X" ) {
    return false;
  }
  return move;
}
