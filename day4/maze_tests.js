...
describe('Maze.getStartPosition()', function() {
  ...
  mazes.forEach(function(m) {
    ...
    it("new Maze(" + JSON.stringify(m) + ").getStartPosition() -> " + pos, function() {
      if (found) expect(maze.getStartPosition())
        .toEqual(pos);
...
describe('Maze.tryMove()', function() {
  ...
  // 1,0 should be false
  it("new Maze([['S'], ['E']]).tryMove(0, 0, 'down') -> false", function() {
    expect(new Maze([['S'], ['E']])
        .tryMove(0, 0, 'down'))
      .toEqual(false);
  });
  // 1,0 should be false
  it("new Maze([['S'], ['E']]).tryMove(1, 0, 'up') -> false", function() {
    expect(new Maze([['S'], ['E']])
        .tryMove(1, 0, 'up'))
      .toEqual(false);
  });
  ...
  //typo 1,2 should be false
  it("new Maze([['S', ' ', 'E'], ['X', 'X', ' ']]).tryMove(1, 2, 'up') -> false", function() {
    expect(new Maze([['S', ' ', 'E'], ['X', 'X', ' ']])
        .tryMove(1, 2, 'up'))
      .toEqual(false);
  });
});
