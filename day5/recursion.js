...
recursion.cache = {'0': 0, '1': 1};

recursion.fibonacci = function(number) {
	if(recursion.cache.hasOwnProperty(number)){
		return recursion.cache[number];
	}
	var ans =  recursion.fibonacci(number - 1) + recursion.fibonacci(number - 2);
	recursion.cache[number] = ans;
	return ans;
}
