...
describe("Tests for expectation types", function() {
  ...
  // 'not.' (including the period) before 'toBe(' below. --> added not
  ...
  // offers another expectation type: toEqual(). ---> updated toBe to toEqual

  it("array [1] should be equal to [1]", function() {
    expect([1]).toEqual([1]); // Fix this test case
  });

  // We can use 'not' to negate toEqual() checks too. --> added not
  it("array [1] should not be equal to [2]", function() {
    expect([1]).not.toEqual([2]); // Fix this test case
  });

  // .toBeTruthy() checks if a value is truthy --> updated toBe to toBeTruthy
  it("1 should be truthy", function() {
    expect(1).toBeTruthy(); // Fix this test case
  });

  // .toBeFalsy() does the oppsite of .toBeTruthy() --> updated toBe to toBeFalsy
  it("0 should be falsy", function() {
    expect(0).toBeFalsy(false); // Fix this test case
  });
  ...
  // Wrap error() in a function --> make sure toThrow is after function declaration {}
